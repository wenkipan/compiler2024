...
static inline void _dealCond(BasicBlock *_exiting, BasicBlock *nextBB, Loop *loop, IRCopy &copyer)
{
    BasicBlock *nwexiting = copyer.get_mapbb(_exiting);
    ...
    if (loop->is_BBinLoop(((Branch *)_exiting->get_last_instrution())->get_trueBB()))
    {
...
void loopunroll::Unroll(Loop *loop, const int times, Function *p_func)
{
    ...
    for (int i = 0; i < times; ++i)
    {
        ...
        if (i != times - 1)
        {
            ...
            _dealCond(lpexiting, nextHeader, loop, copyer[i]);
            for (PHINode *Phi : *loop->get_header()->get_phinodes())
            {
