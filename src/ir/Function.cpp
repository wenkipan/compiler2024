    ...
    : GlobalValue(p_func), currentbblabel(0)
{
    ...
    list_for_each(p_node, &p_func->param)
    {
        ...
        Instrution *p_instr = new Store(p_param, p_vreg, entry_block);
...
template <typename T>
void _IRGen(p_ast_block p_ast_block, Function *func)
{
    T *Gener = new T(func, func->get_entryBB());
    ...
    : GlobalValue(_p_func), currentbblabel(0)
{
    ...
    _IRGen<GenFunction>(_p_ast_block, this);
}

void Function::Val_pushBack(Value *p_val)
{
    values.emplace_back(p_val);
}

void Function::BB_addTail(BasicBlock *p_block)
{
